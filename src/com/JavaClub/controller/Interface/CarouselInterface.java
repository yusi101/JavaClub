 ...
 */
@Controller
@RequestMapping(value="/Interface/carouselInterface")
public class CarouselInterface extends BaseController{
	...
	public String queryCarouselInfo() throws Exception{
		Page page = new Page();
		...
		try {
			...
			List<Map<String, Object>> carouselInfo=new ArrayList<>();
			
			//如果pageSize存在，并且大于1，则该接口分页，否则不分页
	        if(StringUtil.isInt(pd.getString("pageSize"))){
	            Verification.SetPageList(pd, page);//设置page的分页
	            carouselInfo= carouselService.queryCarouseInfoListPage(page);
	            Verification.getPageMessage(resultMap, page);
	        }else{
	            carouselInfo= carouselService.queryCarouseInfo(pd);
	        }
	        
			MapReplaceUtils.handleLsitMapData(carouselInfo);
			resultMap.put("carouselInfo",carouselInfo);
		}catch (Exception e) {
