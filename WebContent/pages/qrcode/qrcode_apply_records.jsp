...
<body>
	<div class="container-fluid">
		<div class="row-fluid">
			<h4 class="title">牌照流水查询</h4>
			<form action="${pageContext.request.contextPath}/qrCodeController/queryQRCodeApplyInfo" method="post" name="queryEntnameInfo" id="queryEntnameInfo" class="form-horizontal" style="margin: 10px 0; height: 30px;">
					...
					<c:if test="${!empty qrList }">
						<c:forEach items="${qrList }" var="list" varStatus="status">
							<tr>
								...
								<td><button class="btn btn-detail" onclick="openApplyDets('${list.ID }','${list.PID }','${list.PRIPID }')"><i class="fa fa-navicon"></i>&nbsp;详情</button></td>
							</tr>
						</c:forEach>
					</c:if>
					<c:if test="${empty qrList }">
