...
<body>
	<div class="container-fluid">
		<div class="row-fluid">
			<h4 class="title">牌照出入库查询</h4>
			<form action="${pageContext.request.contextPath}/qrCodeController/queryQRCodeRecordlistPage" method="post" name="queryEntnameInfo" id="queryEntnameInfo" class="form-horizontal" style="margin: 10px 0; height: 30px;">
			...
			<table class="table table-condensed table-bordered table-hover tab">
				<thead>
					<tr>
						...
						<th width="10%">牌照</th>
						<th width="10%">打印数量</th>
						<th width="10%">领取处理人</th>
						<th width="17%">处理时间</th>
						<th width="10%">牌照批次号</th>
					</tr>
				</thead>
				<tbody id="tbody">
					<c:if test="${!empty qrList }">
						<c:forEach items="${qrList }" var="list" varStatus="status">
							<tr>
								...
								<td><fmt:formatDate type="both" value="${list.RECEIVE_TIME }" /></td>
								<td>${list.BATCHNO }</td>
							</tr>
						</c:forEach>
					</c:if>
