...
//删除用户
function deleteEmailsMods(ID){
	layer.confirm('是否删除？', {icon: 3, title:'提示'}, function(index){ 
		//执行删除
		$.ajax({
			url : contextPath + '/emailsModsController/deleteEmailsMods',
			timeout : 300000,
			dataType : "json",
			type : "post",
			data : {
...
function openadd(){
	layer.open({
		...
		area: ['650px', '70%'],  //大小
		content: contextPath + '/emailsModsController/toCreateemailsMods'
	}); 
}
...
function query(Kid){
	layer.open({
		...
		maxmin: true,//是否显示最大化按钮
		area: ['650px', '70%'],  //大小
		content:  contextPath + '/emailsModsController/queryEmailsModsById?KID='+Kid
	}); 
}
