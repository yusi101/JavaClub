...
<body>
	<div class="container-fluid">
		<div class="row-fluid">
			<h4 class="title">用户操作日志</h4>
			</div>
			<form action="${pageContext.request.contextPath}/userLogController/userLoglistPage" method="post" name="queryBrandInfo" id="queryBrandInfo" class="form-horizontal" style="margin: 10px 0; height: 30px;">
			...
			<table class="table table-condensed table-bordered table-hover tab">
				<thead>
					<tr>
						...
						<th class="center" width="20%">功能模块</th>
						<th class="center" width="10%">姓名</th>
						<th class="center" width="10%">IP地址</th>
						<th class="center" width="15%">操作时间</th>
						<th class="center" width="10%">操作</th>
					</tr>
				</thead>
				<tbody>
					<c:if test="${not empty userLogList }">
						<c:forEach items="${userLogList }" var="list" varStatus="status">
							<tr>
								<td class="center">${status.index+1 }</td>
								<td class="center">${list.MODULENAME}</td>
								<td class="center">${list.CREATEUSERNAME}</td>
								<td class="center" >${list.IP}</td>
								<td class="center"><fmt:formatDate value="${list.CREATEDTIME}" pattern="yyyy年MM月dd日  HH时mm分ss秒" /></td>
								<td class="dropdown">
	...
	<script type="text/javascript">
		...
		function query(ID){
			layer.open({
				...
				area: ['700px', '70%'],  //大小
				content: '${pageContext.request.contextPath}/userLogController/queryUserLogById?ID='+ID
			});
		}
