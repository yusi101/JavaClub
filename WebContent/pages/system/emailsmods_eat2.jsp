	...
	<div class="form-horizontal">
		<table id="table_bug_report" class="table table-striped table-bordered table-hover" style="width: 580px; text-align: center; margin: 10px auto 15px;">
			<tr>
				...
				<td style="text-align: right;">
					<input type="text" id="modsname" name="modsname" style="width: 80%;" maxlength="30" value="${EmailsMods_list.EMAIL_TITLE}" />
				</td>
			</tr>
			<tr>
				<td style="text-align: right;">
				...
				<td style="text-align: right;">
					<textarea rows="3" cols="3" style="width: 80%;" maxlength="200" id="remark" name="remark">${EmailsMods_list.EMAIL_CTX}</textarea>
				</td>
			</tr>
			<tr>
				<td>
				...
				<td>
					<input type="hidden" id="hID" name="hID" value="${strID }" />
					<input type="file" name="uploadFile" id="uploadFile">
					<div name="Filelist" id="Filelist">
					<c:forEach items="${EmailsModsAttr_list }" var="list" varStatus="status">
								<span class="fileName">${list.FILENAME}</span>
			...
			<input type="hidden" id="ID" name="ID" style="width: 63%;" maxlength="30" value="${EmailsMods_list.ID }" />
			<input type="radio" name="type" id="type" value="0" <c:if test="${EmailsMods_list.ACTIVE =='0' }">checked="checked"</c:if>/>&nbsp;不激活
				<input type="radio" name="type" id="type" value="1" <c:if test="${EmailsMods_list.ACTIVE =='1' }">checked="checked"</c:if>/>&nbsp;激活&nbsp;&nbsp;&nbsp;
			</td>
		</table>
	...
	<script>
			...
			.uploadify(
					{
						...
						'onUploadSuccess' : function(file, data,
							...
							$.ajax({
								url : contextPath + '/emailsModsController/updateEmailsModsById',
								timeout : 300000,
								dataType : "json",
								type : "post",
								data : {
		...
		function submit() {
			...
			if($("#stu").val() == '1' && $("input[name='type']:checked").val() != '1'){
				layer.tips('邮件模板必须有一个为激活状态', '#type',{tips:[1, '#78BA32'],tipsMore: true});
				return;
			}
			ShowLoading();
			if($("#uploadFileList").find(".uploadify-queue-item").length <= 0){
				...
				$
						.ajax({
							url : contextPath
									+ '/emailsModsController/updateEmailsModsById',
							timeout : 300000,
							dataType : "json",
							type : "post",
							data : formdata,
