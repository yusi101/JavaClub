...
var SWFUpload;if(SWFUpload==undefined)
					...
					if (args[0]) {
						...
						if (args[0] == '*') {
							...
							$('#' + settings.queueID).find('.uploadify-queue-item').each(function() {
								...
								$(this).find('.data').removeClass('data').html(' -删除');
							...
							for (var n = 0; n < args.length; n++) {
								...
								$('#' + args[n]).find('.data').removeClass('data').html(' 取消中');
			...
			onSelect : function(file) {
				...
				for (var n in this.queueData.files) {
					...
					if (queuedFile.uploaded != true && queuedFile.name == file.name) {
						/*var replaceQueueItem = confirm('文件名  "' + file.name + '" 已经存在，是否替换？');
						if (!replaceQueueItem) {
				...
				if (settings.itemTemplate == false) {
					settings.itemTemplate = '<div id="${fileID}" class="uploadify-queue-item">\
						...
						<div class="data" ></div><span class="fileName" title="${fileName}">(${fileSize})${fileName}</span>\
						</div>';
				}
				...
				if ($.inArray('onSelectError', settings.overrideEvents) < 0) {
					...
					case SWFUpload.QUEUE_ERROR.QUEUE_LIMIT_EXCEEDED:
						if (settings.queueSizeLimit > errorMsg) {
							...
							this.queueData.errorMsg += '\n选择的文件的数量超过了队列大小限制 (' + settings.queueSizeLimit + ').';
						}
						break;
