...
import com.JavaClub.util.Tools;
...
public class PagePlugin implements Interceptor {
    ...
    public void  createDbLog(BaseStatementHandler delegate,Invocation ivk){
        try {
            ...
            if(parameterObject!=null){
                ...
                if(StringUtil.isNotEmpty(logType)){
                    ...
                    if( user != null){
                        ...
                        try {
                            page=(Page)parameterObject;
                            parameter=getParameter(page.getPd());
                        } catch (Exception e) {
                            parameter="";
                        }
                        ...
                        loginfo.put("PARAMETERINFO", parameter);
                        loginfo.put("CREATEUSERID", user.getUSER_ID());
                        loginfo.put("REMARK", getSqlInfo(sql, parameter));
                        SaveDB(loginfo, connection);
                    }else{
                        /*PageData pd=new PageData();
	                    try {
	                        pd=(PageData)parameterObject;
	                        parameter=getPageDate(pd);
	                        loginfo.put("ID", UuidUtil.get32UUID());
	                        loginfo.put("IP", pd.getString("IP"));
	                        loginfo.put("LOGTYPE", "login");
	                        loginfo.put("SQLINFO", sql);
	                        loginfo.put("CREATEDTIME", new Date()); 
	                        loginfo.put("SQLINFO", sql);
	                        loginfo.put("ADDRESS", sql);
	                        loginfo.put("REMARK", getPageDate(pd));
    ...
    public void SaveDB(Map loginfo,Connection connection){
        try {
            String countSql = "INSERT INTO SYS_LOG_DBINFO(ID,IP,LOGTYPE,SQLINFO,PARAMETERINFO,CREATEUSERID,CREATETIME,REMARK) "
                    + " VALUES(?,?,?,?,?,?,now(),?)"; 
            PreparedStatement pstmt = connection.prepareStatement(countSql);
            pstmt.setString(1, loginfo.get("ID").toString());
            pstmt.setString(2, loginfo.get("IP").toString());
            pstmt.setString(3, loginfo.get("LOGTYPE").toString());
            pstmt.setString(4, loginfo.get("SQLINFO").toString());
            pstmt.setString(5, loginfo.get("PARAMETERINFO").toString());
            pstmt.setString(6, loginfo.get("CREATEUSERID").toString());
            pstmt.setString(7, loginfo.get("REMARK").toString());
            pstmt.executeUpdate();  
        } catch (Exception e) {
        }
    }
    ...
    public String getParameter(Map map){
        ...
        if(!map.isEmpty()){
            for (Object key : map.keySet()) {
                pagedate+=map.get(key)+"@";
            }
            pagedate=pagedate.substring(0,pagedate.length()-1);
        }
        return pagedate;
    }
    public static String getSqlInfo(String sql,String parameter){
        String parameters[]=parameter.split("@");
        for (int i = 0; i < parameters.length; i++) {
            sql=sql.replaceFirst("[?]", "'"+parameters[i]+"'");
        }
        return sql;
    ...
    public String getLoggerType(String sql){
        ...
        for(String type:logTypes){
            if(sql.toUpperCase().startsWith(type.toUpperCase())){
                if(!type.toUpperCase().equals("INSERT") && sql.toUpperCase().indexOf("SYS_LOG_DBINFO")<0){
                    return type;
                }
            }
        }
