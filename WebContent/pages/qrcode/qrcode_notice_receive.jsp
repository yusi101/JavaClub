...
<body>
	<div class="container-fluid">
		<div class="row-fluid">
			<h4 class="title">牌照出库</h4>
			<form action="${pageContext.request.contextPath}/qrCodeController/queryQRCodeReceivelistPage" method="post" name="queryEntnameInfo" id="queryEntnameInfo" class="form-horizontal" style="margin: 10px 0; height: 30px;">
				<div style="width: 10%; float: left;">
					<input type="button" class="btn btn-add" onclick="receiveQRCode();" value="领取">
				</div>
				...
				<tbody id="tbody">
					<c:if test="${!empty qrList }">
						<c:forEach items="${qrList }" var="list" varStatus="status">
							<tr>
								<td>
									<input type="checkbox" name="ids" value="${list.RELATION_ID },${list.ENTNAME },${list.REGNO }" style="vertical-align: text-top"/>
								</td>
								<td>${list.ENTNAME }</td>
								...
								<%-- <td><fmt:formatDate type="both" value="${list.SEND_TIME }" /></td> --%>
								<td>${list.BATCHNO }</td>
							</tr>
						</c:forEach>
					</c:if>
